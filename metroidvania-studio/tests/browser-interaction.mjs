...
const localRoot = path.resolve(repositoryRoot, 'metroidvania-studio/.local');
...
assert.ok(localRelative && !localRelative.startsWith('..') && !path.isAbsolute(localRelative),
  'The destructive browser fixture must stay beneath metroidvania-studio/.local.');
...
await mkdir(path.join(root, '.local/logs'), { recursive: true });
...
function assertWorkspaceUnchanged(after, before, message = 'Read-only input must preserve authoring state.') {
  // Passive export/health updates may change revision without an authoring command.
  assert.equal(after.documentRevision, before.documentRevision, message);
  assert.equal(after.catalogRevision, before.catalogRevision, message);
  assert.deepEqual(after.selection, before.selection, message);
}
...
try {
  ...
  // Export status may advance the UI revision while these requests are in flight.
  assert.equal(afterHttpBoundary.documentRevision, beforeHttpBoundary.documentRevision, 'Rejected HTTP bodies must preserve document revision.');
  ...
  assertWorkspaceUnchanged(afterInvalidBrush, brush16, 'Invalid brush size must preserve the revision.');
  ...
  assertWorkspaceUnchanged(afterBlankRoomInput, beforeBlankRoomInput, 'A blank room coordinate must not silently become zero.');
  ...
  assert.equal(await page.locator('#camera-preview').count(), 0, 'The old view-switch button is replaced by Preview.');
  const cameraDrawOps = await page.evaluate(() => window.__mapDrawOps);
  assert.equal(cameraDrawOps.dashedCameraFrames, 0);
  assert.deepEqual(cameraDrawOps.cameraLabels, []);
  await page.locator('#preview-maximize').click(); await nextPaint();
  assert.equal(await page.locator('#game-preview-canvas').getAttribute('data-view-mode'), 'camera');
  const previewCanvas = await page.locator('#game-preview-canvas').boundingBox();
  await page.mouse.click(previewCanvas.x + previewCanvas.width / 2, previewCanvas.y + previewCanvas.height / 2);
  const commandsBeforePreviewKeys = commandActions.length;
  for (const shortcut of ['u', 'Delete', 'Backspace', 'Control+z', 'Control+y', 'Control+c', 'Control+x', 'Control+v', 'Control+a'])
    await page.keyboard.press(shortcut);
  assert.equal(commandActions.length, commandsBeforePreviewKeys);
  assert.deepEqual((await state()).document, beforeCameraPreview.document);
  await page.keyboard.press('Escape'); await nextPaint();
  assert.equal(await page.locator('#preview-maximize').getAttribute('aria-pressed'), 'false');
  assert.deepEqual(await page.locator('#map-canvas').boundingBox(), editCanvas);
  await page.locator('#tabs button').nth(1).click();
  assert.equal(await page.locator('#game-preview').isVisible(), false);
  await page.locator('#tabs button').first().click();
  assert.equal(await page.locator('#game-preview').isVisible(), true);
  // The dedicated browser-game-preview suite also covers live strokes, independent
  // panning, cache reuse, frame fitting, DPI, collapse persistence and camera settings.
  await page.locator('#preview-collapse').click();
  checks.push('Preview is read-only and restores the editor layout without changing camera or document');
  ...
  assertWorkspaceUnchanged(heldStroke, beforeStroke, 'A held tile stroke must not contact or mutate the server.');
  ...
  assertWorkspaceUnchanged(heldBent, beforeBent, 'A held bent stroke must remain client-local.');
  ...
  assertWorkspaceUnchanged(heldErase, beforeErase, 'A held erase stroke must not contact or mutate the server.');
  ...
  assertWorkspaceUnchanged((await state()), beforeEmptyErase, 'Erasing an empty cell must remain a client-side no-op.');
  ...
  assertWorkspaceUnchanged((await state()), beforeExactBoundary, 'An exact-limit stroke must remain client-local until pointerup.');
  ...
  assertWorkspaceUnchanged(afterExpandedOverflow, beforeExpandedOverflow, 'A brush-expanded overflow must preserve server state.');
  ...
  assertWorkspaceUnchanged((await state()), beforeHugeJump, 'A capped pointer jump must remain client-local until pointerup.');
  ...
  assertWorkspaceUnchanged(afterHugeJump, beforeHugeJump, 'A cancelled pointer jump must preserve the server revision.');
  ...
  assertWorkspaceUnchanged((await state()), beforeDelayedStroke, 'A stroke during a delayed writer must not mutate the server.');
  ...
  assertWorkspaceUnchanged((await state()), beforePan); checks.push('middle drag pans without editing');
  ...
  assertWorkspaceUnchanged(afterBlankObjectInput, beforeBlankObjectInput, 'A blank object transform must not silently become zero.');
  ...
  assertWorkspaceUnchanged(afterMiniShortcuts, beforeMiniView, 'MiniMap authoring shortcuts must not change server state.');
  assert.deepEqual(commandActions, [], 'MiniMap authoring shortcuts must send no command.');
  await page.screenshot({ path: path.join(root, '.local/logs/ExternalEditor-MiniMap-Test.png') });
  ...
  assertWorkspaceUnchanged((await state()), beforeStandaloneShortcuts, 'Standalone MiniMap shortcuts must remain read-only.');
  ...
  await page.screenshot({ path: path.join(root, '.local/logs/ExternalEditor-InteractionFailure.png') });
