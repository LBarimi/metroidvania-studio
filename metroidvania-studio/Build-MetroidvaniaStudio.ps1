...
param([switch]$CheckOnly, [Parameter(ValueFromRemainingArguments=$true)][string[]]$RemainingArguments)
$ErrorActionPreference = 'Stop'
$studioRoot = Split-Path $PSScriptRoot -Parent
. (Join-Path $PSScriptRoot 'Runtime-Tools.ps1')
$options = Merge-StudioOptions -Options @{ CheckOnly=[bool]$CheckOnly } -Arguments $RemainingArguments -Allowed @('CheckOnly')
