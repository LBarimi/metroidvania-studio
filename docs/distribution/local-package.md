...
The resulting archive is `builds/npm/metroidvania-studio-1.3.1.tgz`. The build checks the source version, matching npm/MCP metadata, required notices, and package contents. `builds/npm/latest.json` records the archive inventory and npm integrity hash. These are local build outputs.

Install the local archive in a separate test folder:

```sh
npm install --offline --ignore-scripts --no-audit --no-fund ./metroidvania-studio-1.3.1.tgz
