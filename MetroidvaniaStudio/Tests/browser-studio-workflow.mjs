...
try {
  ...
  await page.screenshot({ path: path.join(repository, 'Logs/Studio-About.png') });
  ...
  for (const [language, caption] of [['JA','ファイル'],['ZH_CN','文件'],['ZH_TW','檔案'],['RU','Файл'],['KR','파일'],['EN','File']]) {
    ...
    await page.screenshot({ path: path.join(repository, 'Logs/Studio-' + language + '.png') });
  }
  ...
  await page.screenshot({ path: path.join(repository, 'Logs/Studio-Selection.png') });
  ...
  await page.screenshot({ path: path.join(repository, 'Logs/Studio-Workflow-Failure.png') });
