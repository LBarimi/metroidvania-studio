...
export function buildDocs(output = path.join(root, 'builds/docs')) {
  ...
  const outputName = name => name === 'docs/index.md' ? 'index.html' : name.replace(/^docs\//, '').replace(/\//g, '--').replace(/\.md$/, '.html').toLowerCase();
  ...
  const pages = sources.filter(name => name.endsWith('.md')).map(name => {
    ...
    return { name, title, source, href: names.get(name), ...markdown(source, name) };
  });
  const groups = [['Start here', ['docs/index.md', 'docs/cli/release-downloads.md', 'docs/cli/quick-start.md', 'docs/mcp/setup.md']], ['Lua scripts', ['docs/scripting/quick-start.md', 'docs/scripting/api-reference.md', 'docs/scripting/execution-limits.md']], ['API reference', ['docs/api/index.md', 'docs/api/operations.md', 'docs/api/queries.md', 'docs/api/live-api.md']], ['Reference & distribution', ['docs/cli/commands.md', 'docs/mcp/tools.md', 'metroidvania-studio/contracts/FORMAT.md', 'docs/distribution/local-package.md', 'docs/distribution/publication.md', 'docs/validation.md']]];
  ...
  for (const page of pages) {
    ...
    const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Lua, CLI, MCP and editing API documentation for Metroidvania Studio."><title>${escape(page.title)} · Metroidvania Studio</title><link rel="icon" href="studio-icon.svg"><link rel="stylesheet" href="docs.css"><script src="search-index.js" defer></script><script src="docs.js" defer></script></head><body><a class="skip" href="#content">Skip to content</a><header><a class="brand" href="index.html"><img src="studio-icon.svg" width="28" height="28" alt="">Metroidvania Studio <span>Docs</span></a><span class="version">${escape(version)}</span><a href="https://github.com/LBarimi/metroidvania-studio">GitHub</a></header><div class="layout"><aside class="sidebar"><label for="search">Search documentation</label><input id="search" type="search" placeholder="Lua, rooms, headless…" autocomplete="off"><div id="results" hidden aria-live="polite"></div><nav aria-label="Documentation">${nav}</nav></aside><main id="content">${page.html}<footer>Metroidvania Studio ${escape(version)} · Documentation works offline.</footer></main><aside class="toc"><span>On this page</span>${page.toc.map(h => `<a href="#${h.id}">${escape(h.text)}</a>`).join('')}</aside></div></body></html>`;
  ...
  for (const name of sources.filter(name => !name.endsWith('.md'))) {
    ...
    writeFileSync(path.join(output, names.get(name)), bytes);
  }
  ...
  console.log(`Built ${pages.length} offline documentation pages.`);
  return pages;
}
