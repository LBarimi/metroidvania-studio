...
export function packRelease(state) {
  ...
  run(node, ['metroidvania-studio/build-web.mjs']);
  const output = path.join(root, '.local/release');
  mkdirSync(output, { recursive: true });
  const staging = mkdtempSync(path.join(output, 'staging-'));
  const bundle = path.join(staging, 'metroidvania-studio');
  mkdirSync(path.join(bundle, 'metroidvania-studio/server'), { recursive: true });
  run(dotnet, ['publish', 'metroidvania-studio/server/MetroidvaniaStudio.Server.csproj', '--configuration', 'Release',
    '--self-contained', 'false', '-p:UseAppHost=false', '-p:UseSharedCompilation=false', '-p:DebugType=None',
    '--output', path.join(bundle, 'metroidvania-studio/server')]);
  run(dotnet, ['publish', 'metroidvania-studio/launcher/MetroidvaniaStudio.Launcher.csproj', '--configuration', 'Release',
    '--self-contained', 'false', '-p:UseAppHost=false', '-p:UseSharedCompilation=false', '-p:DebugType=None',
